...
def transaction_to_txid(data):
    bytes = binascii.unhexlify(data)
    # SHA-256 (first round)
    hash1 = hashlib.sha256(hashlib.sha256(bytes).digest()).digest()
    txid = binascii.hexlify(hash1).decode('utf-8')
    print(txid)  

    # print result (reverse byte order)
    return (''.join(reversed([txid[i:i+2] for i in range(0, len(txid), 2)])))
# from the STATIC_BLOCK_DATA, returns a `partial_header` and `nonce` touple.
txids = []

for c in STATIC_BLOCK_DATA["transactions"]: 
    txids.append(transaction_to_txid(c))
print(txids)

def build_header_template():

    header_list = [STATIC_BLOCK_DATA["version"], STATIC_BLOCK_DATA["prev_block_hash"], calculate_merkle_root(txids), STATIC_BLOCK_DATA["time"], STATIC_BLOCK_DATA["bits"]]
...
def mine():
    ...
    with open("../block.txt", "w") as e: 
        e.write(f"{header +"02"+ STATIC_BLOCK_DATA["transactions"][0]+STATIC_BLOCK_DATA["transactions"][1]}")
    while True:
